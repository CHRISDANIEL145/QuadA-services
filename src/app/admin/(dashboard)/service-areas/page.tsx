...
import { getAdminServiceAreas } from '@/actions/admin'
import { cn } from '@/lib/utils'

export const metadata: Metadata = { title: 'Service Areas' }

export default async function AdminServiceAreasPage() {
  const areas = await getAdminServiceAreas()

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[#0D1526] mb-1">Service Areas</h1>
      ...
      <div className="bg-white rounded-2xl border border-[#E5E9F2] overflow-hidden">
        {areas.length === 0 ? (
          <div className="text-center py-20 text-[#6B6254]">
            <p className="text-lg font-medium mb-2">No service areas found</p>
                ...
                {areas.map((area: any, i: number) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      i === areas.length - 1 && 'border-b-0'
                    )}
                  >
                    <td className="px-5 py-4">
                      <div className="font-medium text-[#0D1526] text-sm">{area.name}</div>
                    </td>
                    <td className="px-5 py-4 text-sm text-[#6B6254]">
                      {area.state}
                    </td>
                    <td className="px-5 py-4">
                      {area.is_active ? (
